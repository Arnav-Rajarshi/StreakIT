...
def validate_user(password: str, user_name: str = None, email: str = None) -> bool:
    with db.get_connection() as conn:
        with conn.cursor() as cur:
            if user_name:
                user = crud_db.get_UserDetails(user_name=user_name)
            elif email:
                user = crud_db.get_UserDetails(email=email)
            else:
                return False
...
def get_HabitConfig(uid, HabitName:str):
    return crud_db.get_HabitConfig(uid, HabitName)


def get_HabitCache(hid):
    return crud_db.get_HabitCache(hid)
...
def main():
    if validate_user(password, user_name=username):
        global user
        user=crud_db.get_UserDetails(user_name=username)
        print(f"User {username} validated successfully.")
        global habit_config
        habit_config = get_HabitConfig(user.uid, habit_name)
        ...
        if habit_config:
            ...
            habit_cache = get_HabitCache(habit_config.hid) 
            if habit_cache:
...
main()
print("-*-"*25)
get_HabitCache(habit_config.hid)